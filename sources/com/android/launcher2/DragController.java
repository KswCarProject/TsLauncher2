...
public class DragController {
    ...
    public void startDrag(View v, Bitmap bmp, DragSource source, Object dragInfo, int dragAction, Rect dragRegion, float initialDragViewScale) {
        int[] loc = this.mCoordinatesTemp;
        this.mLauncher.getDragLayer().getLocationInDragLayer(v, loc);
        startDrag(bmp, loc[0] + v.getPaddingLeft() + ((int) (((((float) bmp.getWidth()) * initialDragViewScale) - ((float) bmp.getWidth())) / 2.0f)), loc[1] + v.getPaddingTop() + ((int) (((((float) bmp.getHeight()) * initialDragViewScale) - ((float) bmp.getHeight())) / 2.0f)), source, dragInfo, dragAction, (Point) null, dragRegion, initialDragViewScale);
    ...
    public void startDrag(Bitmap b, int dragLayerX, int dragLayerY, DragSource source, Object dragInfo, int dragAction, Point dragOffset, Rect dragRegion, float initialDragViewScale) {
        ...
        this.mDragObject.dragComplete = PROFILE_DRAWING_DURING_DRAG;
    ...
    public Bitmap getViewBitmap(View v) {
        v.clearFocus();
        v.setPressed(PROFILE_DRAWING_DURING_DRAG);
        boolean willNotCache = v.willNotCacheDrawing();
        v.setWillNotCacheDrawing(PROFILE_DRAWING_DURING_DRAG);
    ...
    public void cancelDrag() {
        if (this.mDragging) {
            ...
            this.mDragObject.deferDragViewCleanupPostAnimation = PROFILE_DRAWING_DURING_DRAG;
            this.mDragObject.cancelled = true;
            this.mDragObject.dragComplete = true;
            this.mDragObject.dragSource.onDropCompleted((View) null, this.mDragObject, PROFILE_DRAWING_DURING_DRAG, PROFILE_DRAWING_DURING_DRAG);
        }
        endDrag();
    }
    ...
    public void onAppsRemoved(ArrayList<String> packageNames, Context context) {
        if (this.mDragObject != null) {
            ...
            if (rawDragInfo instanceof ShortcutInfo) {
                ...
                while (it.hasNext()) {
                    String pn = it.next();
                    if (dragInfo != null && dragInfo.intent != null && dragInfo.getPackageName().equals(pn)) {
                        cancelDrag();
                        return;
                    }
                }
    ...
    private void endDrag() {
        if (this.mDragging) {
            ...
            clearScrollRunnable();
            boolean isDeferred = PROFILE_DRAWING_DURING_DRAG;
    ...
    public boolean dispatchUnhandledMove(View focused, int direction) {
        if (this.mMoveTarget == null || !this.mMoveTarget.dispatchUnhandledMove(focused, direction)) {
    ...
    private void handleMoveEvent(int x, int y) {
        ...
        this.mLastTouch[0] = x;
        this.mLastTouch[1] = y;
        int delay = this.mDistanceSinceScroll < slop ? RESCROLL_DELAY : SCROLL_DELAY;
    ...
    public void forceMoveEvent() {
        if (this.mDragging) {
            handleMoveEvent(this.mDragObject.x, this.mDragObject.y);
        }
    }
    ...
    private void dropOnFlingToDeleteTarget(float x, float y, PointF vel) {
        ...
        boolean accepted = PROFILE_DRAWING_DURING_DRAG;
    ...
    private void drop(float x, float y) {
        ...
        this.mDragObject.dragSource.onDropCompleted((View) dropTarget, this.mDragObject, PROFILE_DRAWING_DURING_DRAG, accepted);
    }
