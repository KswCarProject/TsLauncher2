...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i2 = 0; i2 < this.mDragOutlineAnims.length; i2++) {
            final InterruptibleInOutAnimator anim = new InterruptibleInOutAnimator((long) duration, 0.0f, toAlphaValue);
        ...
        this.mShortcutsAndWidgets.setCellDimensions(this.mCellWidth, this.mCellHeight, this.mWidthGap, this.mHeightGap);
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params, boolean markCells) {
        ...
        Resources res = getResources();
        if (res.getInteger(R.integer.myiconback) == 0) {
            if (child instanceof BubbleTextView) {
                ((BubbleTextView) child).setTextColor(res.getColor(R.color.workspace_icon_text_color));
            }
        } else if (child instanceof BubbleTextView) {
            BubbleTextView bubbleChild = (BubbleTextView) child;
            if (this.mIsHotseat) {
                bubbleChild.setBackground((Drawable) null);
            } else {
                bubbleChild.setBackgroundResource(R.drawable.icon_back);
            }
            bubbleChild.setTextColor(res.getColor(R.color.workspace_icon_text_color));
        } else if (child instanceof FolderIcon) {
            child.setBackgroundResource(R.drawable.icon_back);
        }
                    ...
                    if (ignoreOccupied) {
                        ...
                        while (true) {
                            if (i >= minSpanX) {
                                ...
                                for (int j2 = 0; j2 < minSpanY; j2++) {
                                    if (occupied[x + i][y + j2]) {
                                        break;
                                    }
                                }
                                i++;
    ...
    private int[] findNearestArea(int cellX, int cellY, int spanX, int spanY, int[] direction, boolean[][] occupied, boolean[][] blockOccupied, int[] result) {
        ...
        int bestDirectionScore = ExploreByTouchHelper.INVALID_ID;
        int countX = this.mCountX;
        int countY = this.mCountY;
        for (int y = 0; y < countY - (spanY - 1); y++) {
            for (int x = 0; x < countX - (spanX - 1); x++) {
                ...
                while (true) {
                    if (i >= spanX) {
                        float distance = (float) Math.sqrt((double) (((x - cellX) * (x - cellX)) + ((y - cellY) * (y - cellY))));
                        ...
                        if ((((direction[0] == curDirection[0] && direction[0] == curDirection[0]) || 0 == 0) && Float.compare(distance, bestDistance) < 0) || (Float.compare(distance, bestDistance) == 0 && curDirectionScore > bestDirectionScore)) {
                            bestDistance = distance;
    ...
    public boolean createAreaForResize(int cellX, int cellY, int spanX, int spanY, View dragView, int[] direction, boolean commit) {
        ...
        ItemConfiguration swapSolution = simpleSwap(pixelXY[0], pixelXY[1], spanX, spanY, spanX, spanY, direction, dragView, true, new ItemConfiguration(this, (ItemConfiguration) null));
    ...
    public int[] createArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY, View dragView, int[] result, int[] resultSpan, int mode) {
        ...
        ItemConfiguration swapSolution = simpleSwap(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, this.mDirectionVector, dragView, true, new ItemConfiguration(this, (ItemConfiguration) null));
        ItemConfiguration noShuffleSolution = findConfigurationNoShuffle(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, dragView, new ItemConfiguration(this, (ItemConfiguration) null));
