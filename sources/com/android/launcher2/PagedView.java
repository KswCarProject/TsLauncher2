...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public PagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mTouchState = 0;
        this.mForceScreenScrolled = DEBUG;
        ...
        this.mDeferScrollUpdate = DEBUG;
        this.mIsPageMoving = DEBUG;
        this.mIsDataReady = DEBUG;
        this.mHasScrollIndicator = true;
        this.mShouldShowScrollIndicator = DEBUG;
        this.mShouldShowScrollIndicatorImmediately = DEBUG;
        this.mScrollingPaused = DEBUG;
        this.mNoScrollPage = -1;
        this.mCanLoop = DEBUG;
        this.mbLooping = DEBUG;
        this.finalPage = 0;
        this.mPageWidth = 0;
        this.hideScrollingIndicatorRunnable = new Runnable() {
            public void run() {
                PagedView.this.hideScrollingIndicator(PagedView.DEBUG);
            }
        };
    ...
    public void setCurrentPage(int currentPage) {
        ...
        if (getChildCount() != 0) {
            ...
            this.finalPage = this.mCurrentPage;
    ...
    public boolean computeScrollHelper() {
        if (this.mScroller.computeScrollOffset()) {
            ...
            if (this.mDeferLoadAssociatedPagesUntilScrollCompletes) {
                ...
                this.mDeferLoadAssociatedPagesUntilScrollCompletes = DEBUG;
            }
    ...
    public void screenScrolled(int screenCenter) {
        ...
        boolean isInOverscroll = (this.mOverScrollX < 0 || this.mOverScrollX > this.mMaxScrollX) ? true : DEBUG;
    ...
    public void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            ...
            int leftScreen = 0;
            View currPage = getPageAt(0);
            while (leftScreen < pageCount - 1 && (currPage.getX() + ((float) currPage.getWidth())) - ((float) currPage.getPaddingRight()) < ((float) getScrollX())) {
                leftScreen++;
                currPage = getPageAt(leftScreen);
            }
            int rightScreen = leftScreen;
            View currPage2 = getPageAt(rightScreen + 1);
            while (rightScreen < pageCount - 1 && currPage2.getX() - ((float) currPage2.getPaddingLeft()) < ((float) (getScrollX() + screenWidth))) {
                rightScreen++;
                currPage2 = getPageAt(rightScreen + 1);
            }
            range[0] = leftScreen;
            range[1] = rightScreen;
            return;
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                if (!this.mCanLoop || rightScreen != leftScreen) {
                    ...
                    this.mForceDrawAllChildrenNextFrame = DEBUG;
                } else {
                    int x = (int) (getPageAt(1).getX() - getPageAt(0).getX());
                    int offset = pageCount * this.mPageWidth;
                    if (leftScreen == 0) {
    ...
    public boolean onInterceptTouchEvent(android.view.MotionEvent r12) {
        ...
        L_0x0028:
            ...
            int r7 = com.android.launcher2.DefaultWorkspace.noscroll
        ...
        L_0x006c:
            ...
            android.widget.Scroller r7 = r11.mScroller
            r7.abortAnimation()
            boolean r7 = r11.mbLooping
            if (r7 == 0) goto L_0x0080
            r11.mbLooping = r6
            int r7 = r11.finalPage
            r11.setCurrentPage(r7)
        L_0x0080:
            int r7 = r11.mTouchState
    ...
    public void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        if (getCurrentPage() != this.mNoScrollPage && (pointerIndex = ev.findPointerIndex(this.mActivePointerId)) != -1) {
    ...
    public float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if (this.mCanLoop) {
            if (this.mCurrentPage == 0 && page == getChildCount() - 1) {
                delta += this.mPageWidth * getChildCount();
            } else if (this.mCurrentPage == getChildCount() - 1 && page == 0) {
                delta -= this.mPageWidth * getChildCount();
            }
        }
        ...
        switch (ev.getAction() & MotionEventCompat.ACTION_MASK) {
            ...
            case 1:
                if (this.mTouchState == 1) {
                    ...
                    boolean isSignificantMove = ((float) Math.abs(deltaX)) > ((float) pageWidth) * SIGNIFICANT_MOVE_THRESHOLD ? true : DEBUG;
                    this.mTotalMotionX += Math.abs((this.mLastMotionX + this.mLastMotionXRemainder) - x2);
                    boolean isFling = (this.mTotalMotionX <= 25.0f || Math.abs(velocityX) <= this.mFlingThresholdVelocity) ? DEBUG : true;
                    boolean returnToOriginalPage = DEBUG;
                    ...
                    if ((isSignificantMove && deltaX > 0 && !isFling) || (isFling && velocityX > 0)) {
                        if (!this.mCanLoop || this.mCurrentPage > 0) {
                            this.finalPage = returnToOriginalPage ? this.mCurrentPage : this.mCurrentPage - 1;
                        } else {
                            this.finalPage = getChildCount() - 1;
                        }
                        snapToPageWithVelocity(this.finalPage, velocityX);
                    } else if ((!isSignificantMove || deltaX >= 0 || isFling) && (!isFling || velocityX >= 0)) {
                        snapToDestination();
                    } else {
                        if (!this.mCanLoop || this.mCurrentPage < getChildCount() - 1) {
                            this.finalPage = returnToOriginalPage ? this.mCurrentPage : this.mCurrentPage + 1;
                        } else {
                            this.finalPage = 0;
                        }
            ...
            case 2:
                if (this.mTouchState != 1) {
                    if (DefaultWorkspace.noscroll == 0) {
    ...
    public void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (this.mCanLoop) {
            if (this.mCurrentPage == 0 && whichPage2 == getChildCount() - 1) {
                this.mbLooping = true;
                delta -= this.mPageWidth * getChildCount();
            } else if (this.mCurrentPage == getChildCount() - 1 && whichPage2 == 0) {
                this.mbLooping = true;
                delta += this.mPageWidth * getChildCount();
            }
        }
        if (Math.abs(velocity) < this.mMinFlingVelocity) {
            snapToPage(whichPage2, delta, PAGE_SNAP_ANIMATION_DURATION);
            return;
        }
    ...
    public void loadAssociatedPages(int page) {
        loadAssociatedPages(page, DEBUG);
    }
        ...
        if (this.mContentIsRefreshable && page < (count = getChildCount())) {
            ...
            for (int i2 = 0; i2 < count; i2++) {
                if ((i2 == page || !immediateAndOnly) && lowerPageBound <= i2 && i2 <= upperPageBound && this.mDirtyPageContent.get(i2).booleanValue()) {
                    ...
                    syncPageItems(i2, z);
                    this.mDirtyPageContent.set(i2, Boolean.valueOf(DEBUG));
                }
            }
        }
    }
    ...
    public void invalidatePageData() {
        invalidatePageData(-1, DEBUG);
    }

    /* access modifiers changed from: protected */
    public void invalidatePageData(int currentPage) {
        invalidatePageData(currentPage, DEBUG);
    }
    ...
    public void flashScrollingIndicator(boolean animated) {
        ...
        showScrollingIndicator(animated ? DEBUG : true);
        ...
        if (getChildCount() > 1 && isScrollingIndicatorEnabled()) {
            ...
            if (this.mScrollIndicator != null) {
                ...
                this.mScrollIndicatorAnimator.addListener(new AnimatorListenerAdapter() {
                    private boolean cancelled = PagedView.DEBUG;
    ...
    private void updateScrollingIndicatorPosition() {
        if (isScrollingIndicatorEnabled() && this.mScrollIndicator != null) {
            ...
            int indicatorPos = ((int) (((float) (trackWidth - indicatorSpace)) * Math.max(0.0f, Math.min(1.0f, ((float) getScrollX()) / ((float) maxScrollX))))) + this.mScrollIndicatorPaddingLeft;
